...
private:
	...
	AudioManager()
	{
		int result = Mix_OpenAudio(
			44100, //Standar frequency that works on most systems
	...
	AudioManager(const AudioManager& am) = delete;
	AudioManager& operator= (const AudioManager& val) = delete;
...
public:
	...
	void LoadClip(std::string name)
	{
		...
		Mix_Chunk* loadedChunk = Mix_LoadWAV(("resources/audio/sfx/" + name + ".mp3").c_str());
	...
	inline void LoadSong(std::string name)
	{
		...
		Mix_Music* loadedMusic = Mix_LoadMUS(("resources/audio/sfx/" + name + ".mp3").c_str());
	...
	inline void Mute(){	_muted = true;}

	inline void UnMute(){_muted = false;}
